    ...
    if random.random() < P_NOISE:
        ...
        if noise_type == 'gaussian':
            ...
            img = np.clip(
                ...
                img.astype(np.float32) * noise / 255.0,
                0,
                255
            ).astype(np.uint8)
...
def rust_augment(img):
    if random.random() < P_RUST and img.ndim == 3:
        ...
        img = (
            img.astype(np.float32) *
            (1 - rust_mask[:, :, np.newaxis] * blend) +

            rust_color *
            rust_mask[:, :, np.newaxis] * blend
        )
...
def background_augment(img):
    if random.random() < P_BG_CHANGE and img.ndim == 3:
        ...
        img = (
            img.astype(np.float32) * fg_mask[:, :, np.newaxis] +
...
def process_one_drawing(args):
    ...
    for variant in range(num_variants):
        ...
        meta.append((out_name, class_idx))
...
def main():
    parser = argparse.ArgumentParser(
        description="铆钉图纸数据增强"
    )

    parser.add_argument(
        '--num',
        type=int,
        default=AUG_PER_DRAWING,
        help=f'每张图纸生成数量（默认 {AUG_PER_DRAWING}）'
    )
    ...
    parser.add_argument(
        '--output',
        type=str,
        default=str(Config.AUGMENTED_DIR)
    )
    ...
    print(f"[增强] 类别数: {num_classes}")
    print(f"[增强] 每类生成: {args.num}")
    print(f"[增强] 进程数: {args.workers}")
    ...
    print(f"[增强] 完成，共生成 {len(all_samples)} 张")
